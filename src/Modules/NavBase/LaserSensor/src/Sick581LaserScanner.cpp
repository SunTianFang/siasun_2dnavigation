...
#define ROS_SIMU_LASER

void* LCMRecvTaskSick(LPVOID pParam)
{
...
cSick581LaserScanner::cSick581LaserScanner(int fAngRes, float fStartAng, float fEndAng):
    CRangeScanner(fAngRes, fStartAng, fEndAng, SICK581)
{
    m_iNetType = -1;
    m_iLaserPort = 0;
    m_iScanFrequency = 0;
...
{
    m_plcm = lcm_create( "udpm://224.0.0.1:7667?ttl=1" );
...
#ifdef ROS_SIMU_LASER
        intensity.push_back(  ( (unsigned int)(laserdata->intensities[i]) )*15); //test
#else
        intensity.push_back(  ( (unsigned int)(laserdata->intensities[i]) ));
#endif
    }
                                 ...
                                 const int iSamplesPerScan)
{
    ...
    if(pthread_create(&m_Thread,&attr,LCMRecvTaskSick,reinterpret_cast<LPVOID>(this)) != 0)
    {
        std::cout<<"Creat Sick581LaserScanner Pthread Failed"<<std::endl;
        return FALSE;
    }
    else
        std::cout<<"Creat Sick581LaserScanner Pthread OK"<<std::endl;
