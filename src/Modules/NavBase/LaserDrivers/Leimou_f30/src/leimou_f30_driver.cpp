...
Intelly::Intelly():is_connected_(false), is_scanning_(false)
{
    ...
    socket_data_type = FRAME_TYPE_INTELLY_FORMAL;
    cache_thread_ptr_ = nullptr;
    m_hostIp = "";
...
bool Intelly::StartScan()
{
    ...
    if (!is_connected())
    {
//        std::cerr << this->m_hostIp << "is_connected(): " << is_connected() << std::endl;
        return false;
    }
    ...
    {
//        std::cerr << this->m_hostIp << "is_scanning_: " << is_scanning_ << std::endl;
        return true;
    }
    ...
    {
//        std::cerr << this->m_hostIp << "(cache_thread_ptr_ == nullptr)" << std::endl;
        return false;
    }
    else
        cache_thread_ptr_->detach();
    is_scanning_ = true;
//    std::cerr << this->m_hostIp << " is_scanning_: " << is_scanning_  << " cache_thread_ptr_: " << cache_thread_ptr_.get() << std::endl;
    // Sleep 2 senconds.
    // sleep(2.0);
    return true;
}
...
bool Intelly::GrabScanData(ScanData& scan_data)
{
    ...
    {
//        std::cerr << this->m_hostIp << " scan_data_len_: " << scan_data_len_ << " is_scanning_: " << is_scanning_ << std::endl;
        return false;
    }
    ...
    buf_mutex_.unlock();
//    std::cerr << this->m_hostIp << " flag: " << flag << std::endl;
    return flag;
}
...
//        std::cerr << " socket_fd_: " << socket_fd_ << std::endl;
...
//            ioctl(socket_fd_, FIONBIO, &ul); //设置为非阻塞模式.
            ...
            {
                std::cerr << __FUNCTION__ << " line:" << __LINE__ << " socket error: "<<strerror(errno)<<std::endl;
            ...
            if(ret == true)  // succeed.
            {
                ...
                std::cout  << __FUNCTION__ << " line:" << __LINE__
                           << " Connected to host: " << host << " ,at port: " << port << " succeeded.\n";
                return true;
            }
            else    // Failed.
            {
                std::cerr  << __FUNCTION__ << " line:" << __LINE__
                           << " Connected to host: " << host << " ,at port: " << port << " Failed!\n";
                return false;
            }
            first_flag = false;
        }
...
bool Intelly::DisConnect()
{
    if (is_connected_)
    {
        close(socket_fd_);
        is_connected_ = false;
        std::cerr << __FUNCTION__ << " line:" << __LINE__ << " "  << this->m_hostIp << " Disconnet socket!" << std::endl;
    }
...
void Intelly::CacheScanData()
{
    ...
    while(is_scanning_)
    {
        ...
        {
            is_scanning_ = false;
            std::cerr << __FUNCTION__ << " line: " << __LINE__ << "Receive data error!" << std::endl;
            break;
        }
//            std::cerr << __FUNCTION__ << " line: " << __LINE__ << " " << this->m_hostIp  << " recv_len: " << recv_len << std::endl;

        // Get single package.
        buf_mutex_.lock();
        l_u8Ret = DealSocketDataInPackage(recv_buf, recv_len, scan_data_buf_, scan_data_len_);
//        std::cerr << __FUNCTION__ << " line: " << __LINE__ << " " << this->m_hostIp
//                  << " recv_len: " << recv_len << " scan_data_len_: " << scan_data_len_ << std::endl;
        buf_mutex_.unlock();
