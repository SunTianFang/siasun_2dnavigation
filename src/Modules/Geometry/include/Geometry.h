...
class DllExport CPnt
{
  ...
  public:
    ...
    // 判断一个给定的点是否“触碰”到该点
    bool PointHit(const CPnt &pt, float fDistGate) const;
    ...
    // 在屏幕上绘制该点(如 mmSize>0, 则点的尺寸随比例尺的大小自动绽放)
    void Draw(CScreenReference &ScrnRef, QPainter *pPainter, QColor color, int nPointSize = 1,
              int nLineWidth = 0, float mmSize = 0);
#endif
};
...
class DllExport CAngle
{
  ...
  public:
     ...
     */
    static float NormalizeAngleDifference(float subtrahend, float minuend);
...
class DllExport CPolyRegion
{
  ...
  public:
    ...
    // 根据参数生成对象
    bool Create(int nCount, CPnt *pPnt);
...
#ifdef _MFC_VER
    // 在屏幕上绘制此直线
    void Draw(CScreenReference &ScrnRef, CDC *pDC, COLORREF crColor, int nWidth = 1,
              int nPointSize = 1, bool bBigVertex = false);
#endif
};
...
class DllExport CPosture : public CPnt
{
  ...
  public:
    ...
    void SetAngle(const CAngle &ang) { fThita = ang.m_fRad; }

    void SetAngle(float fAngle) { fThita = CAngle::NormAngle(fAngle); }

    void SetPosture(const CPosture &pst) { GetPostureObject() = pst; }
    ...
    void ReverseAngle() { fThita = CAngle::NormAngle(fThita + PI); }
...
#ifdef _MFC_VER
    // 在屏幕上绘制此姿态
    void Draw(CScreenReference &ScrnRef, CDC *pDC, COLORREF crLine, COLORREF crFill,
              int nCircleRadiusMm, int nArrowLenMm, int nWidth = 1);

#elif defined QT_VERSION

    // 在屏幕上绘制此姿态
    void Draw(CScreenReference &ScrnRef, QPainter *pPainter, QColor crLine, QColor crFill,
              int nCircleRadiusMm, int nArrowLenMm, int nWidth = 1, QString str = "",
...
class DllExport CVelocity
{
  ...
  public:
    ...
    CVelocity(float fXLin, float fYLin, float fAng)
    {
        fXLinear = fXLin;
        fYLinear = fYLin;
        fLinear = sqrt(fXLinear * fXLinear + fYLinear * fYLinear);
        fAngular = fAng;
    }

    // Default constructor
    CVelocity() { Initialize(); }
...
class DllExport vector_velocity
{
  public:
    float vel_x;
    float vel_y;
  ...
  public:
    ...
    void SetZero() { vel_x = vel_y = vel_angle = 0; }
};
...
class DllExport CTransform : public CPosture
{
  public:
    ...
    void Create(float _x, float _y, float _angle) { CPosture::Create(_x, _y, _angle); }
...
class DllExport CLineBase
{
  ...
  public:
    ...
    // 通过对点云的线性回归求得最优拟合线
    virtual bool CreateFitLine(const CPnt *sp, int num);
    ...
    // 求得直线外一直线到此直线的投影
    bool GetProjectLine(const CLine &Line, CLine &projectedLine) const;
...
class DllExport CLine : public CLineBase
{
  public:
    ...
    CAngle m_angSlant;    // Slant angle 倾斜角
  ...
  public:
    ...
    // 改变线段的长度(沿两端延长/缩短)
    bool Extend(float fDist1, float fDist2);
    ...
    //
    bool Intersect(const CLine &line2, float *px, float *py, bool *onSegment1, bool *onSegment2,
                   float fSmallGate = 1e-6) const;
    ...
    // 计算该直线到指定点的距离
    float DistanceToPoint(bool bIsSegment, const CPnt &pt, float *pLambda = NULL,
                          CPnt *pFootPoint = NULL) const;
    ...
    // 计算本线段到另一线段的投影
    bool GetProjection(const CLine &another, CLine &lineProj, float &fProjDist,
                       float &fProjLen) const;
    ...
    // 在屏幕上绘制此直线
    void Draw(CScreenReference &ScrnRef, CDC *pDC, COLORREF crColor, int nWidth = 1,
              int nPointSize = 1, bool bBigVertex = false, int nPenStyle = PS_SOLID);
#elif defined QT_VERSION
    // 在屏幕上绘制此直线
    void Draw(CScreenReference &ScrnRef, QPainter *pPainter, QColor crColor, int nWidth = 1,
              int nPointSize = 1, bool bBigVertex = false, int nPenStyle = Qt::SolidLine);
#endif
};
...
class DllExport CRectangle
{
  private:
    ...
    CPnt m_ptLeftBottom;     // 矩形左下角位置
    CPnt m_ptRightTop;       // 矩形右上角位置

  public:
    // 矩形内/外部点的分类
    enum {
        OUTSIDE_POINT = 0,      // 0 - 矩形外部点
        CENTER_POINT,           // 1 - 矩形中心点
        LEFT_TOP_POINT,         // 2 - 矩形左上角顶点
        RIGHT_TOP_POINT,        // 3 - 矩形右上角顶点
        LEFT_BOTTOM_POINT,      // 4 - 矩形左下角顶点
        RIGHT_BOTTOM_POINT,     // 5 - 矩形右下角顶点
        LEFT_MIDDLE_POINT,      // 6 - 矩形左边线中点
        RIGHT_MIDDLE_POINT,     // 7 - 矩形右边线中点
        TOP_MIDDLE_POINT,       // 8 - 矩形上边线中点
        BOTTOM_MIDDLE_POINT,    // 9 - 矩形下边线中点
        BORDER_POINT,           // 10 - 矩形边框上的其它点
        OTHER_INSIDE_POINT      // 11 - 矩形内部的其它点
    };
  ...
  public:
    ...
    CRectangle &GetRectangleObj() { return *this; }
    ...
    bool operator==(const CRectangle &rect);

    void operator=(const CRectangle &rect);

    // 判断一个给定的点是否“触碰”到该矩形
    int PointHit(const CPnt &pt, float fDistGate) const;
    ...
    // 在屏幕上绘制此矩形
    void Draw(CScreenReference &ScrnRef, CDC *pDC, COLORREF crColor, int nWidth = 1,
              int penStyle = 1, bool bFill = false, COLORREF crFill = 0);

#elif defined QT_VERSION
    // 在屏幕上绘制此矩形
    void Draw(CScreenReference &ScrnRef, QPainter *pPainter, QColor crColor, int nWidth = 1,
              int penStyle = Qt::SolidLine, bool bFill = false, QColor crFill = Qt::black);
...
class DllExport CCircle
{
  ...
  public:
    ...
    bool IntersectLineAt(const CLine &Line, CPnt &ptNear, float &fDist) const;
    ...
    void Draw(CScreenReference &ScrnRef, CDC *pDC, COLORREF crColor, int nWidth,
              int nPenStyle = PS_SOLID, bool bFill = false, COLORREF crFill = 0);

#elif defined QT_VERSION
    // 在屏幕上绘制此圆
    void Draw(CScreenReference &ScrnRef, QPainter *pPainter, QColor crColor, int nWidth,
              int nPenStyle = 0, bool bFill = false, QColor crFill = Qt::black);
...
class DllExport CArc
{
  ...
  public:
    // The constructor
    CArc(const CPnt &ptCenter, const CPnt &ptStart, const CPnt &ptEnd,
...
class DllExport CSpp : public CArc
{
  public:
    // The constructor
    CSpp(CPnt &ptCenter, CPnt &ptStart, CPnt &ptEnd, CTurnDir TurnDir = COUNTER_CLOCKWISE);
...
class DllExport CSpline : public CSpp
{
  ...
  public:
    // The constructor
    CSpline(CPnt &ptCenter, CPnt &ptStart, CPnt &ptEnd, CTurnDir TurnDir = COUNTER_CLOCKWISE);
