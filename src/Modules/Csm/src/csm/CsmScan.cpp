...
CCsmScan::CCsmScan(const CScan& Scan)
{
	...
	for (int i = 0; i < nCount; i++)
	{
        ...
        float reading = Scan.m_pPoints[j].r;    // jzz: 注意单位，这里默认是m
        // 此处修改reading的获取方式，上边是原始点云的reading，在局部坐标系下（帧间，模板适用），下边通过x，y坐标重新计算reading在全局坐标系下（适用建图）
        // 因为变成全局坐标系计算时，reading也要相对应为全局坐标系，而不是局部坐标系，此时target，source都在全局坐标系下输入到csm中，匹配结果为当前真实位姿。
        // 注意:如果用此处方式获取reading值，那么csm匹配出来的结果是真实位姿，而不在是位姿变化量（△），外层要跟着修改.
//        float reading = sqrt(pow(Scan.m_pPoints[j].x, 2) + pow(Scan.m_pPoints[j].y, 2));
...
//        m_pPoints[i].theta = min_theta + i * (max_theta - min_theta) / (m_nRays - 1);
        ...
        float fAngle = CAngle::NormAngle((float)atan2(Scan.m_pPoints[j].y, Scan.m_pPoints[j].x)); // 0 ~ 2PI
