...
class RealTimeCorrelativeScanMatcher2D {
 public:
  ...
  double Match_1(const transform::Rigid2d& initial_pose_estimate,
               const sensor::PointCloud& point_cloud, const Grid2D& grid,
                int &highscore_num) const;
  ...
  //  by sam
  void MatchWithQuality(const transform::Rigid2d& initial_pose_estimate,
               const sensor::PointCloud& point_cloud, const Grid2D& grid
               ,double &grid_quality,double &point_quality,LaserParam &param, vector<int>& vtOmit) ;
  ...
  void ScoreCandidates_1(const Grid2D& grid,
                       const std::vector<DiscreteScan2D>& discrete_scans,
                       const SearchParameters& search_parameters,
                       std::vector<Candidate2D>* candidates,int &highscore_num) const;
  void ScoreCandidatesWithQuality(const Grid2D& grid,
                       const std::vector<DiscreteScan2D>& discrete_scans,
                       const SearchParameters& search_parameters,
                       std::vector<Candidate2D>* candidates,double &grid_quality,double &point_quality) ;
