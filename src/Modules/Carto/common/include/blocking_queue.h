...
#ifndef COMMON_BLOCKING_QUEUE_H_
#define COMMON_BLOCKING_QUEUE_H_
...
//	printf("queue_size_=%d, deque_.size()= %d\n",queue_size_, deque_.size());

    lock.Await([this]() REQUIRES(mutex_) { return   queue_size_ == kInfiniteQueueSize || deque_.size() <  queue_size_; }); //QueueNotFullCondition(); });
   ...
   // printf(" Push deltime = %f\n", (time2.tv_sec - time3.tv_sec)*1000 + (double)(time2.tv_usec -time3.tv_usec)/1000  );  
...
/*  bool PushWithTimeout(T t, const common::Duration timeout) {
    ...
    if (!lock.AwaitWithTimeout(
            [this]() mutable REQUIRES(mutex_) { return queue_size_ == kInfiniteQueueSize || deque_.size() <  queue_size_; },  //QueueNotFullCondition(); },
            timeout)) {
      return false;
    }
    deque_.push_back(std::move(t));
  ...
  T Pop() {
   ...
   // lock.Await([this]() REQUIRES(mutex_) { return  !QueueEmptyCondition(); });
	...
	lock.Await([this]() REQUIRES(mutex_) { return !deque_.empty(); });
 ...
 /* T PopWithTimeout(const common::Duration timeout) {
    MutexLocker lock(&mutex_);
    if (!lock.AwaitWithTimeout(
            [this]() REQUIRES(mutex_) { return !deque_.empty(); },           // !QueueEmptyCondition(); },
            timeout)) {
      return nullptr;
    }
    T t = std::move(deque_.front());
...
#endif  // COMMON_BLOCKING_QUEUE_H_
