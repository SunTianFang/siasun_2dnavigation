...
//从FeaturMap地图中读取数据 在给pad
bool CLocalizeFactory::GetScanMatchLocArea()
{
    if(plan_ == NULL || plan_->size() == 0)
    {
        std::cout  << "paln is null" << std::endl;
        return false;
    }
    ...
    areas.reserve(plan_->size()); // 预留空间，假设每个元素都有效
    params.reserve(plan_->size());
    std::cout <<"plan_->size(): " << plan_->size() << std::endl;
    for(int i = 0; i < plan_->size(); ++i)
    {
        CLocalizationRect *q = dynamic_cast<CLocalizationRect*>(plan_->at(i));
        if(q == NULL || q->methodId_[0] != 3) // 3:带表当前是珊格定位
            continue;
        vector<int> param;
        areas.emplace_back(std::initializer_list<float>{
            q->GetLeftTopPoint().x,
            q->GetLeftTopPoint().y,
            q->GetRightBottomPoint().x,
            q->GetRightBottomPoint().y
        });
         if(q->methodId_[1] == 4) //slam模式
            {
                param.push_back(1);
            }
            else
            ...
            CScanMatchParam *paramScanmatch = (CScanMatchParam*)q->param_[0];
            param.push_back(paramScanmatch->m_localMode);      //获取长走廊模式
            param.push_back(paramScanmatch->m_topVisionMode);  //获取顶视模式
            param.push_back(i);  // 预留项1, 用0初始化  说要存一个区域id
            param.push_back(0);  // 预留项2, 用0初始化
    ...
    LCMTask myTask;
    myTask.SendLocateAreaBoxesAndParams(67,area_num,areas,params); //已经获取数据准备给pad了 commandid = 67 
    ...
    for(int i = 0; i < area_num; ++i)
    {
        ...
        std::cout << "params[" << i << "] = " 
                  ...
                  << params[i][2] << " " 
                  << params[i][3] << " "         // 输出预留项1
                  << params[i][4] << std::endl;  // 输出预留项2
        #if defined USE_BLACK_BOX
                    FILE_BlackBox(LocBox, "MultipleAreas have sent to pad!!! :", "area_num: ",area_num,", ",
...
bool CLocalizeFactory::SetScanMatchLocArea(int area_num, const vector<vector<float> > areas, const vector<vector<int>> params)
{
    std::lock_guard<std::mutex> lock1(loc_mtx);
    CLocalizationPlan *Pad_plan_ = plan_;
    ...
    for(int i=0; i < Pad_plan_->size(); i++)
    {
        ...
        if(q->CheckMethodUsed(3)==0)
        {
           if(Pad_plan_->Delete(i))
           {
                std::cout<<"------------------------ Scanmatch plan has been deleted!!!!!!"<<std::endl;
                i--;
           }
        }
    }
    ...
    if(area_num!=areas.size() || area_num!=params.size())
        return false;

    for (int j = 0; j < area_num; j++)
    {
        CRectangle* p = Pad_plan_->CreateRect();
        if (!p->Create( areas[j][0], areas[j][1], areas[j][2], areas[j][3])){
            return false;
        }
        ...
        // 该区域启用scanmatch法
        r->methodId_[0] = 3;   //type[j];
        r->param_[0] = methods_->CreateLocalizationParam(3);
        CScanMatchParam *paramScanmatch = (CScanMatchParam*)r->param_[0];

        if(params[j].at(0) == 1)   //slam
        {
            r->methodId_[1] = 4;
            r->param_[1] = methods_->CreateLocalizationParam(4);
            CSlamLocalizationParam *paramSlam = (CSlamLocalizationParam*)r->param_[1];
            paramSlam->SlamMaxDis = 100;   //允许单次slam最远距离
            paramSlam->SlamTotalDis = 100; //允许单次slam最长距离
        }
        paramScanmatch->m_localMode = params[j].at(1);  // 0-正常模式 1-单点模式x 2-单点模式y 3-长廊模式x 4-长廊模式y
        paramScanmatch->m_topVisionMode =  params[j].at(2); //融合顶视模式 0、不用顶视定位 1、只用顶视定位 2、激光融合顶视

        cout << "ready file : " << params[j].at(0) << " " << params[j].at(1) << " " << params[j].at(2) << " "
        << areas[j][0] << " " << areas[j][1] << " " << areas[j][2] << " " << areas[j][3] << std::endl;
        cout<<"paramScanmatch->m_localMode: "<< paramScanmatch->m_localMode <<  endl;
        cout<<"paramScanmatch->m_topVisionMode: " << paramScanmatch->m_topVisionMode <<  endl;

        Pad_plan_->push_back((CRectangle*)r); //需要把内容添加进去

    }

    // 打开 FeatureMap.map 重新保存添加模板后的地图
    FILE *fp = fopen(WORK_PATH"FeatureMap.map", "wb");
    std::cout << "open Featuremap ok!!"<< std::endl;
    if (fp == NULL)
    {
    ...
    if (fwrite(version_, sizeof(char), 8, fp) != 8 || fwrite(date_, sizeof(char), 6, fp) != 6)
    {
        std::cout << "fwrite false" << std::endl;
        fclose(fp);
        return false;
    }
    if(SaveBinary(fp,WORK_PATH))
