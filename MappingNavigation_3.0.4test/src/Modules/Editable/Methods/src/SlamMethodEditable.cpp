...
CSlamMethodEditable::CSlamMethodEditable()
{
    visible_ = true;
...
//   绘制对应于该方法的地图。
//
void CSlamMethodEditable::PlotNormalMap(QPainter *painter, CScreenReference &scrnRef)
{
    if (!visible_)
        return;
...
/*    if(map_==nullptr)
  ...
  //  std::cout<<"  "<<resolution <<" "<<max_x<<" "<<max_y<<std::endl;
...
//   绘制对应于该方法的定位情况。
//
void CSlamMethodEditable::PlotLocalization(QPainter *painter, CScreenReference &scrnRef)
{
#if 0
    QString str="";
...
//   显示对应于该方法的匹配数据。
//
void CSlamMethodEditable::ShowMatchStatus(QPainter *painter, CScreenReference &scrnRef, int curStep,
                                         int totalSteps)
{
/*
