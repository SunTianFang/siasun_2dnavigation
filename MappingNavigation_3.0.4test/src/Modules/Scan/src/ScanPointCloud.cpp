...
//   从二进制文件中读取扫描数据。
//
bool CScanPointCloud::LoadBinary(FILE *fp, float fStartAngle, float fEndAngle, float fMinRange, float fMaxRange, int nLineCount,
                                 int nFileVersion)
{
    // 计算扫描的角分辨率
    ...
    for (int i = 0; i < m_nCount; i++)
    {
        ...
        else
        {
            ...
            r = ur * 2.0f;                // 极径分辨率为2mm
            nIntensity = (int)ui/* * 10*/;    // 强度范围(0-255) (0~2550 //wt_test_intensity 20230221
        }

        if (r < fMinRange * 1000 || r > fMaxRange * 1000)    // r > 65500为临时措施，解决扫描线出现一圈半径为65534的弧的问题
            r = 0;
...
bool CScanPointCloud::SaveBinary(FILE *fp, int nFileVersion)
{
    if (nFileVersion >= 210)
    {
        if (fwrite(&m_dwTimeStamp, sizeof(unsigned int), 1, fp) != 1)
            return false;
    }
    for (int i = 0; i < m_nCount; i++)
    {
        ...
        else
        {
            ...
            unsigned char ui = (unsigned char)(sp.m_nIntensity/* / 10*/);    // 强度用单字节表示 //wt_test_intensity 20230221
