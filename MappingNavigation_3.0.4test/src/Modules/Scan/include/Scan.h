...
{
  public:
    bool m_bVisible;            // 本扫描是否可见(仅用于屏幕显示)
    CPosture m_poseRelative;    // 激光头的相对姿态
    CPosture m_pstScanner;      // 激光头的估算姿态
  ...
  public:
    ...
    // 与给定的参考虑点云进行匹配
    bool PointCloudMatch(const CScan &refScan, CFrame &trans, CCorrList &CorrList, sm_result *pResult = NULL);

    // 根据给定的“强反光门限”值，标记哪些点是属于“强反光”
    void MarkReflectivePoints(int nReflectiveGateValue);

    // 启用新的激光扫描器有效范围
    void ApplyNewScannerAngles(const CDataRangeSet &AngleRange);
...
#ifdef _MFC_VER
    void Plot(CScreenReference &ScrnRef, CDC *pDC, COLORREF crColor, COLORREF crHighLightPoint, bool bShowScanner, COLORREF crScanner,
              int nPointSize = 1);
#elif defined QT_VERSION
    void Plot(CScreenReference &ScrnRef, QPainter *pPainter, QColor crColor, QColor crHighLightPoint, bool bShowScanner, QColor crScanner,
              int nPointSize = 1);
