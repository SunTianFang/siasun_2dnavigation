...
#ifndef MAPPING_ID_H_
#define MAPPING_ID_H_
...
//EIGEN_MAKE_ALIGNED_OPERATOR_NEW
   public:
    ...
    typedef  IdDataReference value_type;
    typedef  int64 difference_type;
    typedef  std::unique_ptr<const IdDataReference> pointer;
    typedef  const IdDataReference& reference;
...
//EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  IdType Append(const int trajectory_id, const DataType& data) {
	...
	// ADD BY LISHEN  
	bool bfind = true;
	typename std::map<int, MapByIndex>::iterator iter;
	iter = trajectories_.find(trajectory_id);
    ...
    trajectory.data_.insert(std::pair<int, DataType>{index, data});
  ...
  void Insert(const IdType& id, const DataType& data) {
    ...
    CHECK(trajectory.data_.insert(std::pair<int, DataType>{GetIndex(id), data}).second);
  }
