...
template <typename T>
T GetYaw(const Eigen::Quaternion<T,Eigen::DontAlign>& rotation) {
...
Eigen::Matrix<T, 3, 1> RotationQuaternionToAngleAxisVector(
    const Eigen::Quaternion<T,Eigen::DontAlign>& quaternion) {
  Eigen::Quaternion<T,Eigen::DontAlign> normalized_quaternion = quaternion.normalized();
...
// vector.
template <typename T>
Eigen::Quaternion<T,Eigen::DontAlign> AngleAxisVectorToRotationQuaternion(
  ...
  return Eigen::Quaternion<T,Eigen::DontAlign>(w, quaternion_xyz.x(), quaternion_xyz.y(),
                              quaternion_xyz.z());
}
