...
int AlgorithmAPI::ShadowsFilter(UserData* scan_in, const ShadowsFilterParam& param)
{
	...
	for (int i = 0; i < scan_in->framedata.data.size() - param.window - 1; i++)
	{
...
int AlgorithmAPI::MedianFilter(UserData* scan_in, const MedianFilterParam& param)
{
	...
	for (int i = param.window; i < scan_in->framedata.data.size() - param.window - 1; i++)
	{
...
int ParseAPI::parse_data_x(int len, unsigned char* buf,UartState *uartstate, RawData& dat, int& consume, int with_chk,int &byte,char*result, CmdHeader *cmdheader,void** fan_segs)
{
