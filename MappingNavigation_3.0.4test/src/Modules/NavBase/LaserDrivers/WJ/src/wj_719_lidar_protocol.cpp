...
//        std::cerr << "..................dataProcess.................." << std::endl;
         ...
         while(m_sdata.m_u32out < m_sdata.m_u32in)
         {
           if(m_sdata.m_acdata[m_sdata.m_u32out] == 0xFF && m_sdata.m_acdata[m_sdata.m_u32out+1] == 0xAA)
           {
             ...
             {
//                 std::cerr << "..................OnRecvProcess before.................." << std::endl;
    ...
    {
//        std::cerr << "..................OnRecvProcess in................." << std::endl;

         if(len > 0)
         {
           if(checkXor(data,len))
           {
//             std::cerr << "..................protocl before................." << std::endl;
             protocl(data,len);
           }
         }
         else
    ...
    {
//        std::cerr << "..................protocl in................." << std::endl;
       if(data[0] == 0xFF && data[1] == 0xAA)
       {
           if((data[16] == 0x01 && data[17] == 0x04))//ScanData 0.3
           {
               //dq
             if (data[2] == 0x04 && (data[3] == 0xC5 || data[3] == 0xCE))
              //if (data[2] == 0x04 && (/*data[3] == 0xC5 ||*/ data[3] == 0xCE))
              {

                  //std::cerr << " data[3]: " << std::hex << data[3] << std::endl;
                 g_n32ScanDatRcvdFrames++;
                 heartstate = true;
                 //cout << "Analysis ScanData" << endl;
                 int l_n32SerialNo = g_n32ScanDatRcvdFrames;//Recieved Scandata Number
...
//                 }
                 if (data[18] == 0x01)
                 {
//                    cout << "Recv ScanData 00" << endl;
                    g_byteScanFrameBank1 = 0x01;
//dq
                    if(data[3] == 0xC5 || data[3] == 0xCE) // ntp
//                    if(data[3] == 0xCE) // ntp
                    {
 //                       cout << "Recv ScanData 01" << endl;
                        uint64_t g_n32TimeStampSec =   (unsigned int)((data[1221] & 0xff) << 24) +                                //时间戳秒
                                    (unsigned int )((data[1222]& 0xff) << 16) +
                                    (unsigned int)((data[1223]& 0xff) << 8) +
                                    (unsigned int)(data[1224] & 0xff );
//                        unsigned int l_ntimeStampSec = (g_n32TimeStampSec -(25567*24*3600));                             //转化为1970年1月1日开始
                        uint32_t g_n32TimeStampSubSec =   (unsigned int)((data[1225] & 0xff) << 24) +                             //时间戳亚秒
                                    (unsigned int )((data[1226]& 0xff) << 16) +
                                    (unsigned int)((data[1227]& 0xff) << 8) +
                                    (unsigned int)(data[1228] & 0xff );
//                        std::cerr << GetTickCount() <<"   g_n32TimeStampSec: " << g_n32TimeStampSec << " g_n32TimeStampSubSec: " << g_n32TimeStampSubSec << std::endl;
//                        unsigned int l_ntimeStampNsec = (unsigned int )(g_n32TimeStampSubSec * 0.23283);
                        if(g_n32TimeStampSec!=0)
                        {
                           m_bsyncstate = true;
                           m_synctime = NTPToUTC(g_n32TimeStampSec, g_n32TimeStampSubSec) - m_diffvalue;

//                           std::cerr << " ^^^^^^^^m_synctime: " << m_synctime <<std::endl;
                        }
                        else
                        {
                            m_bsyncstate = false;
                  ...
                  )
                 {
                     ...
                     m_bfullscan = true;
                   // std::cerr << "*********m_rawtime: " << m_rawtime << std::endl;
...
//                    double l_64nDataRegionZhiY = 0;
                    ...
                    for (int m = 0; m < 14400; m = m + 4)
                    {
                        ...
                        nIntensity = (int)(fScandata719[m + 3] & 0x7F) * 150/*2048*/; //将反光板的光强调整到2500以上 wt_test 20230129
...
//                        std::cerr << dataIndex << " Intensity: " << nIntensity << std::endl;
