...
#define SCANNER_CONNECT_SPAN        10000   //10s

//网络数据类型
#define TCP_DATA 1
#define UDP_DATA 2
enum LaserType { PEPPERL_FUCHS = 0, HOKUYO, SICK , SICK581, WJ716, WJ719,LEIMOUF30,FRR2,LDS_50,LDS_E320_S};
    ...
    unsigned long long GetRawTimeStamp();
                            ...
                            unsigned long long time_raw, unsigned long long time_sync)
        {
            bool bRet = false;
            {
                ...
                //      std::cout<<"i = "<<i<< "  distance[i] = "<<distance[i]<< std::endl;
