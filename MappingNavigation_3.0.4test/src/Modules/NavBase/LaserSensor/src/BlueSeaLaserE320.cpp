...
vector<float> dataE320;
vector<float> intensityE320;
std::mutex mtxE320;
//int last_indx = -1;

bool m_bLaserUnInverted = false;
// 传入回调指针的方式打印
void CallBackMsgE320(int msgtype, void *param,int length)
{
    std::lock_guard<std::mutex> lock(mtxE320);
   ...
   // std::cout<<"msgtype  "<<msgtype<<std::endl;
    ...
    case 1:
    {
        ...
        if (pointdata->type == FRAMEDATA)
        {
             ...
             int startIndex = m_bLaserUnInverted ? pointdata->framedata.data.size() - 1 : 0;
             int endIndex = m_bLaserUnInverted ? -1 : pointdata->framedata.data.size();
             int step = m_bLaserUnInverted ? -1 : 1;

             for (int i = startIndex; i!= endIndex; i += step) {
                 dataE320.emplace_back(pointdata->framedata.data[i].distance * 1000);
                 // 强度归一化
                 if(pointdata->framedata.data[i].confidence > 200) {
                     pointdata->framedata.data[i].confidence = 200;
                 }
                 intensityE320.emplace_back(pointdata->framedata.data[i].confidence);
             }
//             cout << "now is LDS_E320_S blueSeaLaser, LaserInvered:" << m_bLaserUnInverted<< " startIndex" << startIndex <<"endIndex:" << endIndex <<endl;
        }
        else
        {
...
void* LDS_E320ScanProc(LPVOID pParam)
{
     cLDS_E320LaserScanner* oLaserScanner = reinterpret_cast<cLDS_E320LaserScanner*>(pParam);
...
cLDS_E320LaserScanner::cLDS_E320LaserScanner(int fAngRes, float fStartAng, float fEndAng,CLaserScannerParam *cParam):
    CRangeScanner(fAngRes, fStartAng, fEndAng,LDS_E320_S)
{
    m_iNetType = -1;
    //m_LDSScanThread = 0;
    m_LaserScannerIp = "";
    ...
    m_bLaserUnInverted = cParam->g_bLaserUnInverted;
}

cLDS_E320LaserScanner::~cLDS_E320LaserScanner()
{
    Stop();
}

bool cLDS_E320LaserScanner::Stop()
{
    if (!m_bStarted){
        return true;
    }
...
BOOL cLDS_E320LaserScanner::ConnectToLDSLaser(const char*device_name,const int iPort)
{
...
//    int lidarID = lidarSDK->addLidarByPath(cfg_file_name);
    ...
    //传入数据信息的回调函数
    lidarSDK->setCallBackPtr(lidarID, CallBackMsgE320);
    ...
    if(lidarSDK->GetDevInfo(lidarID,&eepromv101))
        CallBackMsgE320(3,&eepromv101,sizeof(EEpromV101));
    Sleep(1000);
    return TRUE;
}

BOOL cLDS_E320LaserScanner::CloseLDSLaserConnection()
{
...
BOOL cLDS_E320LaserScanner::Start(const char*device_name,
...
//    std::cout<<"iPort = "<<iPort<<std::endl;
    ...
    {
        std::cout<<"ConnectToLDE320SLaser Failed: "<<device_name<<std::endl;
        return FALSE;
    }
    else
    {
      std::cout<<"ConnectToLDE320SLaser OK"<<std::endl;
    }
    ...
    if(pthread_create(&m_LDSScanThread,&attr,LDS_E320ScanProc,reinterpret_cast<LPVOID>(this)) != 0)
    {
...
//	获取Urg Scanner数据
//
void cLDS_E320LaserScanner::SupportMeasure()
{
    std::lock_guard<std::mutex> lock(mtxE320);

    unsigned long long  raw_time = GetTickCount();
    if(dataE320.size() <= 0 || intensityE320.size() <= 0)
        return ;
   ...
   /* for(int i = 0; i< data.size(); i++)
    ...
    AddRawPointCloud(dataE320, intensityE320, raw_time, raw_time);
    dataE320.clear();
}
