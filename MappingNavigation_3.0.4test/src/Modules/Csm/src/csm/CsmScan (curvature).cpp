...
CCsmScan::CCsmScan(const CScan& Scan)
{
    ...
    estimate[0] = Scan.m_pstScanner.x;
    estimate[1] = Scan.m_pstScanner.y;
    estimate[2] = Scan.m_pstScanner.fThita;
...
//        m_pPoints[i].theta = min_theta + i * (max_theta - min_theta) / (m_nRays - 1);
        // jzz: 输入数据经过聚类、滤波处理后点不再完整（i=0不是第0线对应的点）因此m_pPoints[i].theta用CScan点的x,y计算
        float fAngle = CAngle::NormAngle((float)atan2(Scan.m_pPoints[i].y, Scan.m_pPoints[i].x)); // 0 ~ 2PI
        m_pPoints[i].theta = fAngle;
        m_pPoints[i].curvature_ = -1;   // jzz: 如果对曲率经过计算，该值应该大于等于0
//        cout << "theta: " << m_pPoints[i].theta << endl;
	}
...
void CCsmScan::SimpleClustering(double threshold)
{
    ...
    // jzz:记录每个cluster（号）的点的个数
    map<int, vector<int>> clusterInfo; // <聚类号（点簇序号）， 点序号（属于该聚类号的所有点序）>
	...
	for (i = 0; i < m_nRays; i++)
	{
        ...
        clusterInfo[_cluster].push_back(i);
        p.cluster = _cluster;
        last_reading = p.readings;
	}

    // 过滤点，如果点簇中点的个数少于filter_num个点，则舍弃。该参数应该设置为可配置
    for (map<int,vector<int>>::const_iterator m_it = clusterInfo.begin(); m_it != clusterInfo.end(); m_it++)
    {
        // 点云滤除，舍弃聚类点小于filter_num的点簇
        if ((int)m_it->second.size() < filter_num)
        {
            for (vector<int>::const_iterator v_it = m_it->second.begin(); v_it != m_it->second.end(); v_it++)
            {
                if (*v_it < m_nRays)
                    m_pPoints[*v_it].valid = false;
            }
        }
        else
        {
            if (filter_num >= 10)
            {
                // 参考loam方法，计算每个点曲率，以每个聚类的点簇为单位
                for (vector<int>::const_iterator v_it = m_it->second.begin()+5; v_it != m_it->second.end()-5; v_it++)
                {
                    float diffX = m_pPoints[*(v_it-5)].points.p[0] + m_pPoints[*(v_it-4)].points.p[0]
                            +  m_pPoints[*(v_it-3)].points.p[0] +  m_pPoints[*(v_it-2)].points.p[0]
                            +  m_pPoints[*(v_it-1)].points.p[0] - 10 *  m_pPoints[*v_it].points.p[0]
                            +  m_pPoints[*(v_it+1)].points.p[0] +  m_pPoints[*(v_it+2)].points.p[0]
                            +  m_pPoints[*(v_it+3)].points.p[0] +  m_pPoints[*(v_it+4)].points.p[0]
                            +  m_pPoints[*(v_it+5)].points.p[0];
                    float diffY = m_pPoints[*(v_it-5)].points.p[1] + m_pPoints[*(v_it-4)].points.p[1]
                            +  m_pPoints[*(v_it-3)].points.p[1] +  m_pPoints[*(v_it-2)].points.p[1]
                            +  m_pPoints[*(v_it-1)].points.p[1] - 10 *  m_pPoints[*v_it].points.p[1]
                            +  m_pPoints[*(v_it+1)].points.p[1] +  m_pPoints[*(v_it+2)].points.p[1]
                            +  m_pPoints[*(v_it+3)].points.p[1] +  m_pPoints[*(v_it+4)].points.p[1]
                            +  m_pPoints[*(v_it+5)].points.p[1];
                    float curvature = diffX * diffX + diffY * diffY;
                    // 给每个点曲率赋值，每个点簇的前5个点和后5个点被舍弃了
                    if (*v_it < m_nRays && curvature < 0.1)
                        m_pPoints[*v_it].curvature_ = 0;
                    else if(*v_it < m_nRays && curvature > 0.1)
                        m_pPoints[*v_it].curvature_ = 1;
                }
            }
        }
    }
