...
class CCsmScan
{
  ...
  private:
    ...
    {
        int j;
        // 注意 !ValidRay(j) 是j为非法的时候才返回 !false = true，即非法时，for循环继续，此目的是找到下一个合法的点，找到合法点，马上就会结束for，return出来
  ...
  public:
    ...
    void PossibleInterval(const double *p_i_w, double max_angular_correction_deg, double max_linear_correction, int *from, int *to, int *start_cell);

    int ValidRay(int i) { return (i >= 0) && (i < m_nRays) && (m_pPoints[i].valid); }

    bool ValidAlpha(int i) { return (m_pPoints[i].alpha_valid != 0); }

    int CountValid();

    int CountInvalid();

    void SetNullCorr(int i) { m_pPoints[i].corr.Clear(); }

    void SetCorr(int i, int j1, int j2) { m_pPoints[i].corr.Set(j1, j2); }

    int NextValidUp(int i) { return NextValid(i, +1); }

    int NextValidDown(int i) { return NextValid(i, -1); }

    bool ValidCorr(int i) { return m_pPoints[i].corr.valid; }
