...
*/
// 3D-NDT变换类，继承于pcl::Registration类
...
class NormalDistributionsTransform : public pcl::Registration<PointSource, PointTarget>
{
  public:
    ...
    /** \brief Typename of pointer to searchable voxel grid leaf. */
    typedef typename TargetGrid::LeafPtr TargetGridLeafPtr;
  ...
  public:
    ...
    {
        if (target_cells_ != NULL)
            delete target_cells_;
    }
  ...
  public:
    ...
    // 根据已设置的目标点云，初始化目标NDT图
    void inline init()
    {
        target_cells_ = new TargetGrid;
    ...
    // 设置用于配准的目标NDT图
    void setTargetCells(VoxelGridCovariance<PointTarget> *targetCells) { target_cells_ = targetCells; }
  ...
  public:
    ...
    // 根据target点云生成的NDT图(含有点的均值和协方差矩阵)
    ...
    // 对应于正态分布的离群率(公式6.7[Magnusson 2009])
    double outlier_ratio_;

    // 用于将"点分布"拟合到"正态分布"的常数参数(公式6.8[Magnusson 2009].)
    double gauss_d1_, gauss_d2_, gauss_d3_;

    // 对输入点云进行变换得到的概率得分(公式6.9,1.10[Magnusson 2009])
    double trans_probability_;

    // 预先计算的角度梯度值-对应于变换向量Jacobian矩阵的角度微分(公式6.19[[Magnusson 2009])
    Eigen::Vector3d j_ang_a_, j_ang_b_, j_ang_c_, j_ang_d_, j_ang_e_, j_ang_f_, j_ang_g_, j_ang_h_;

    Eigen::Matrix<float, 8, 4> j_ang;

    // 预先计算的角度Hessian矩阵-对应于变换向量Hessian矩阵的角度微分(公式6.19[[Magnusson 2009])
