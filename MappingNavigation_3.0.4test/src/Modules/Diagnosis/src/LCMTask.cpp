        ...
        case 31:
         ...
         case 32:   //expandmap
            ...
            case 36:
            {
                if ( PadControlData ->niparams > 1)
                {
                    if(PadControlData->iparams[0] > 0) //定位区域数量大于0
                    {
                        std::cout << "MultipleAreas have been recieved!!!  Num:" << PadControlData ->iparams[0] << std::endl;
                        // 从 PadControlData 结构体获取各参数后准备调用 SetScanMatchLocArea
                        int area_num = PadControlData->iparams[0];  // 区域个数
                         // 验证是否有足够的参数
                        if(PadControlData->ndparams < area_num * 4 || PadControlData->niparams < area_num * 5)
                        {
                            std::cerr << "Not enough parameters provided for all areas." << std::endl;
                            return;
                        }
                        vector<vector<float>> areas(area_num, vector<float>(4)); // 区域坐标点
                        vector<vector<int>>   params(area_num, vector<int>(5));    // 区域参数
                        // 从 PadControlData->dparams 中填充浮点数向量数据
                        for (int i = 0; i < area_num; i++) 
                        {
                            areas[i].resize(4);  
                            areas[i][0] = static_cast<float>(PadControlData->dparams[4 * i]);
                            areas[i][1] = static_cast<float>(PadControlData->dparams[4 * i + 1]);
                            areas[i][2] = static_cast<float>(PadControlData->dparams[4 * i + 2]);
                            areas[i][3] = static_cast<float>(PadControlData->dparams[4 * i + 3]);
                            
                            params[i].resize(5);
                            params[i][0] = PadControlData->iparams[i * 5 + 1];
                            params[i][1] = PadControlData->iparams[i * 5 + 2];
                            params[i][2] = PadControlData->iparams[i * 5 + 3];
                            params[i][3] = 0; //预留参数1
                            params[i][4] = 0; //预留参数2
                        }
                         
                        std::cout << "-----------------------loc info from pad data--------------"<< std::endl;
                        //打印params数据
                        for (int i = 0; i < area_num; i++) 
                        {
                            std::cout << "area " << i << " params: ";
                            for (int j = 0; j < 5; j++) 
                            {
                        ...
                        for (int i = 0; i < area_num; i++) 
                        {
                            std::cout << "area " << i << " areas: ";
                            for (int j = 0; j < 4; j++) 
                            {
                                std::cout << areas[i][j] << " " << std::endl;;
                            }
                        }

                        std::cout << "area_num: " << area_num << std::endl;
                        std::cout << "-----------------------loc info from pad data end--------------"<< std::endl;


                        auto pLocalize = LocalizeFactorySingleton::GetInstance();
                        lcmResult = pLocalize->SetScanMatchLocArea(area_num, areas, params);      

                        if(lcmResult)
                        {
                            LCMTask::GetLcmInstance().SendRecLocSuccessfully();// 通知pad已经写完给定的定位区域和参数数据到Ｆeaturmap中
                            std::cout << "The location areas and information retrieved from the pad have been successfully written into the FeatureMap" << std::endl;
                        }
                         ...
                         for (int i = 0; i < area_num; i++) 
                        {
                            ...
                            params[i][4] = 0; //预留参数2
                                ...
                                FILE_BlackBox(LocBox, "MultipleAreas have been recieved!!! :", "area_num: ",PadControlData->iparams[0],", ",
                                " areas",i,": ",areas[i][0],", ",areas[i][1],", ",areas[i][2],", ",areas[i][3],
                                "  params",i,": ",params[i][0],", ",params[i][1],", ",params[i][2]);
                            #endif
                        }
                    std::cout << "Frist MultipleAreas have been recieved!!!  Num:" <<  PadControlData->iparams[0] << std::endl;
                    }
                    else
                    {
                        std::cout << " commandid 36, ndparams num is error !!!!!\n";
                    }
                }
            }
            break;
            ...
            case 37:
            {
                std::cout << "Frist Received the request from the pad" << std::endl;
                auto pLocalize = LocalizeFactorySingleton::GetInstance();
                if(pLocalize)
                {
                    if(pLocalize->GetScanMatchLocArea())
                    {
                        std::cout << "Received the request from the pad and the data has been sent. " << std::endl;
                    }
                }else{
                    std::cout << "pLocalize initialization failed " << std::endl;
                }
...
// 发送定位区域和参数至pad
void LCMTask::SendLocateAreaBoxesAndParams(int commandId, const int &area_num, const vector<vector<float> > &areas, const vector<vector<int>> &params)
{
    robot_control_t_new  LocateArea;
    int8_t*  iparams = (int8_t*)calloc (5 * area_num + 1, sizeof (int8_t)); //3个是真实使用的 2个是预留项
    double*  dparams = (double*)calloc (4 * area_num, sizeof (double)); //(4：左上右下坐标点)
    iparams[0] = area_num;

    for (int i = 0; i < area_num; ++i)
    {
        int dindex = 4 * i;
        int iparamIndex = 5 * i + 1;

        if (areas[i].size() >= 4)
        {
            dparams[dindex]     = areas[i][0];
            dparams[dindex + 1] = areas[i][1];
            dparams[dindex + 2] = areas[i][2];
            dparams[dindex + 3] = areas[i][3];
        }

        if (params[i].size() >= 3)
        {
            iparams[iparamIndex]     = static_cast<int8_t>(params[i][0]);
            iparams[iparamIndex + 1] = static_cast<int8_t>(params[i][1]);
            iparams[iparamIndex + 2] = static_cast<int8_t>(params[i][2]);
            iparams[iparamIndex + 3] = static_cast<int8_t>(i); //每个区域的id
        }
    }


    Json::Value root; // 根节点，存储所有区域的JSON对象
    Json::Value areaJson; // 单个区域的JSON对象

    for (int i = 0; i < area_num; ++i) {
        Json::Value start, end;

        //  areas 的每个元素有 4 个坐标值：左上角的 x, y 和右下角的 x, y
        int dindex = 4 * i;
        int iparamIndex = 5 * i + 1;

        start["x"] = areas[i].size() >= 4 ? areas[i][0] : 0;
        start["y"] = areas[i].size() >= 4 ? areas[i][1] : 0;
        end["x"] = areas[i].size() >= 4 ? areas[i][2] : 0;
        end["y"] = areas[i].size() >= 4 ? areas[i][3] : 0;

        // 构建单个区域的 JSON 对象
        areaJson["id"] = i + 1; // 从 1 开始编号
        areaJson["slamMode"] = params[i].size() >= 3 ? params[i][0] : 0;
        areaJson["longCorridorMode"] = params[i].size() >= 3 ? params[i][1] : 0;
        areaJson["topViewFusion"] = params[i].size() >= 3 ? params[i][2] : 0;
        areaJson["mapId"] = iparams[iparamIndex + 3];
        areaJson["start"] = start;
        areaJson["end"] = end;

        // 将单个区域的 JSON 对象添加到根节点
        root.append(areaJson);
    }

    //将 Json::Value 对象转换为字符串格式的 JSON
    Json::StreamWriterBuilder builder;
    builder["indentation"] = "  "; // 
    std::string jsonStr = Json::writeString(builder, root);

    // 将Json字符串分配给sparams[0]
    LocateArea.nsparams = 1; // 存储一个字符串
    LocateArea.sparams = (char**)malloc(sizeof(char*)); // 分配存储一个字符串指针的空间
    LocateArea.sparams[0] = (char*)malloc(jsonStr.length() + 1); // 为json字符串分配足够的空间
    strcpy(LocateArea.sparams[0], jsonStr.c_str()); // 将字符串复制sparams[0]中

  
    // 打印或处理 jsonStr
    // std::cout << jsonStr << std::endl;
    LocateArea.commandid = commandId;
    LocateArea.ndparams = 4 * area_num;
    LocateArea.niparams = 5 * area_num + 1;
    LocateArea.nbparams = 0;
    LocateArea.iparams = iparams;
    LocateArea.dparams = dparams;
   
    //打印LocateArea.iparams数据
    for (int i = 0; i < 5 * area_num + 1; ++i)
    {
        printf("LocateArea.iparams[%d] = %d\n", i, iparams[i]);
    }
    //打印LocateArea.dparams数据
    for (int i = 0; i < 4 * area_num; ++i)
    {
        printf("LocateArea.dparams[%d] = %f\n", i, dparams[i]);
    }
    //打印 LocateArea.ndparams 数据
    printf("LocateArea.ndparams = %d\n", LocateArea.ndparams);

    //打印LocateArea.niparams数据
    printf("LocateArea.niparams = %d\n", LocateArea.niparams);

    //打印LocateArea.sparams[0]数据
    printf("LocateArea.sparams[0] = %s\n", LocateArea.sparams[0]);

    //打印LocateArea.commandid
    printf("LocateArea.commandid = %d\n", LocateArea.commandid);

    robot_control_t_new_publish(lcm, "NAVI_UI_COMMAND", &LocateArea);
    free(iparams);
    free(dparams);
    free(LocateArea.sparams[0]); // 首先清理字符串本身
    free(LocateArea.sparams); // 然后清理存储字符串指针的数组
    std::cout << "SendLocateAreaBoxesAndParams end" << std::endl;
}
...
void LCMTask::SendRecLocSuccessfully()
{
    robot_control_t_new  cmd;
