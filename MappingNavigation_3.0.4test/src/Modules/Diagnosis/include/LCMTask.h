...
public:
    ...
    void SendLocateAreaBoxesAndParams ( int commandId, const int &area_num, const vector<vector<float> > &areas, const vector<vector<int>> &params);
    // 导航给 pad 发送心跳, 1s发送一次
    void SendHeart();
